...
#------------------------------------------------------------------------------------------------------------
    ...
    def test_isWorkingHours_Focus(self):        
        ...
        check_result = focusmode.isWorkingHours(target_timestamp)
...
#------------------------------------------------------------------------------------------------------          
    def test_computeFocusMode_Normal_Free(self):
        target_timestamp = datetime.strptime("2021-08-12 19:00:00","%Y-%m-%d %H:%M:%S")
        env_var = "Normal"
        check_result = focusmode.computeFocusMode(env_var, target_timestamp)
...
#------------------------------------------------------------------------------------------------------
    def test_processEnvironVar_MissingEnvVar(self):
        ...
        check_result = focusmode.processEnvironVar(environ_dummy)
...
#------------------------------------------------------------------------------------------------------        
    def test_getUrlFromString(self):
        ...
        check_result = focusmode.getUrlFromString(test_str)
...
"""
        ...
        func_result = focusmode.checkMissingRedirectsFromFile(test_stream,url_list)      
...
"""
        test_stream = io.StringIO(file_text)        
        focusmode.appendRedirects(test_stream, url_list)                
